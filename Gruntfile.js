...
module.exports = function (grunt) {
	function readOptionalJSON(filepath) {
		...
		try {
			data = JSON.parse(
				stripJSONComments(fs.readFileSync(filepath, { encoding: "utf8" }))
			);
		} catch (e) {}
		return data;
	}
	...
	grunt.initConfig({
		...
		karma: {
			...
			main: {
				browsers: (isTravis && travisBrowsers) || [
					"ChromeHeadless",
					"FirefoxHeadless",
				],
			},
	...
	grunt.registerTask("test:slow", ["promises_aplus_tests", "karma:jsdom"]);

	grunt.registerTask("test:prepare", ["qunit_fixture", "babel:tests"]);

	grunt.registerTask("test", ["test:prepare", "test:fast", "test:slow"]);
